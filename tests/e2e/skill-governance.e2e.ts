...
async function waitForRunCompletion(baseUrl: string, jobId: string, timeoutMs: number): Promise<any> {
  const startedAt = Date.now();
  while (Date.now() - startedAt < timeoutMs) {
    const response = await fetch(`${baseUrl}/api/automations/${encodeURIComponent(jobId)}/runs`);
    if (response.ok) {
      const payload = (await response.json()) as { items?: any[] };
      const latest = payload.items?.[0];
      if (latest && latest.triggerSource === 'manual' && latest.status !== 'running') {
        return latest;
      }
    }
    await new Promise((resolve) => setTimeout(resolve, 1000));
  }
  throw new Error(`Timed out waiting for automation run completion after ${timeoutMs}ms.`);
}

async function waitForReportVisible(page: any, timeoutMs: number): Promise<void> {
  const startedAt = Date.now();
  while (Date.now() - startedAt < timeoutMs) {
    const content = await page.locator('textarea').last().inputValue().catch(() => '');
    if (content.includes('"kind": "auto_generated_skill_governance"')) {
      return;
    }
    await page.waitForTimeout(500);
  }
  throw new Error(`Timed out waiting for governance report content after ${timeoutMs}ms.`);
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    assert.ok(governanceJob);
    assert.equal(governanceJob?.readOnly, true);

    await page.goto(baseUrl, { waitUntil: 'domcontentloaded', timeout: 30000 });
    await page.getByTestId('sidebar-open-automations').first().click();
    await page.getByTestId('automation-center').first().waitFor({ timeout: 10000, state: 'visible' });

    const governanceItem = page
      .getByTestId('automation-job-item')
      .filter({ hasText: 'Auto-Generated Skill Governance' })
      .first();
    await governanceItem.waitFor({ timeout: 10000, state: 'visible' });
    await governanceItem.getByTestId('automation-run-now').click();

    const completedRun = await waitForRunCompletion(baseUrl, String(governanceJob?.id), 180000);
    assert.equal(completedRun.triggerSource, 'manual');
    assert.equal(completedRun.status, 'succeeded');

    const reportButton = page.getByTestId('automation-view-report').first();
    await reportButton.waitFor({ timeout: 30000, state: 'visible' });
    await reportButton.click();
    await waitForReportVisible(page, 30000);

    const reportResponse = await fetch(
      `${baseUrl}/api/automations/${encodeURIComponent(String(governanceJob?.id))}/runs/${encodeURIComponent(

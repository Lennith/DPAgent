...
export interface BootstrapDPAgentRuntimeInput {
  config: AgentConfig;
...
export interface BootstrapDPAgentRuntimeResult {
  llmClient: LLMClient;
...
export async function bootstrapDPAgentRuntime(
  input: BootstrapDPAgentRuntimeInput
): Promise<BootstrapDPAgentRuntimeResult> {
  const resolvedLlmRuntime = input.llmRuntime ?? resolveLlmRuntimeConfig({ llmProfiles: input.config.llmProfiles });
  const llmClient = new LLMClient({
    apiKey: resolvedLlmRuntime.apiKey,
    apiBase: resolvedLlmRuntime.apiBase,
    model: resolvedLlmRuntime.model,
    maxTokens: resolvedLlmRuntime.maxOutputTokens ?? input.maxOutputTokens,
    provider: resolvedLlmRuntime.provider,
  ...
  const registerWithDedupe = (tool: Tool, source: ToolSource): void => {
    ...
    if (result.skipped) {
      agentLogger.info(
        `[DPAgent] Tool skipped: name=${tool.name} source=${source} capability=${capability} reason=${result.reason} kept=${result.keptToolName ?? ''}`
      );
      return;
    }
    if (result.replaced) {
      agentLogger.info(
        `[DPAgent] Tool replaced: capability=${capability} next=${tool.name}(${source}) prev=${result.replaced.toolName}(${result.replaced.source})`
      );
    } else {
      agentLogger.info(
        `[DPAgent] Tool registered: name=${tool.name} source=${source} capability=${capability}`
      );
    }
    if (source === 'team') {

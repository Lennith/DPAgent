...
## Daily Workflow
1. Start from latest `master` on a `codex/` branch unless instructed otherwise.
...
6. Commit key nodes with `git commit -s` and a unique `Change-Id`.
...
## Documentation Rule
`doc/` is the single current documentation source. Historical notes may live
under `doc/history/`, but current behavior must be described in the appropriate
current layer: `doc/prd/`, `doc/guide/`, `doc/design/`, `doc/spec/`,
`doc/code/`, or `doc/playbook/`.

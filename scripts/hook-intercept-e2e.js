 ...
 *
 * Usage:
 *   node scripts/hook-intercept-e2e.js --root . --config config.yaml --workspace .tmp/hook-intercept-e2e
 *
...
const ROOT = path.resolve(process.env.DPAGENT_E2E_ROOT || readArg('--root') || path.resolve(__dirname, '..'));
...
const INTERCEPT_PLUGIN = path.join(ROOT, 'plugin', 'examples', 'e2e-intercept-test.cjs');
const DEMO_PLUGIN = path.join(ROOT, 'plugin', 'examples', 'full-hook-demo.cjs');
